...
# Contrib : Patric Proy, J. Jungmann, S. Probst, R. Leroy, M. Berger, R. Forlot
# TODO : 
#################################################################
#
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>)  [-p <port>] [-o <octet_length>] [-t <timeout>] [-V] [-e]\n";
}
...
-e, --exclude
   exclude absent component from check
...
foreach my $key ( keys %$resultstat) {
   ...
   if (defined($o_exclude) and $key =~ /(3|5)/) {
      next;
   }
